...
class SALES_COUNTDOWN_TIMER_Data {
	...
	public function __construct() {
		...
		if ( ! get_option( 'sales_countdown_timer_params' ) || ! count( get_option( 'sales_countdown_timer_params' )['names'] ) ) {
			if ( ! $woo_ctr_settings ) {
		...
		} elseif ( ! $woo_ctr_settings ) {

			$woo_ctr_settings = get_option( 'sales_countdown_timer_params', array() );

		}

		$this->params = apply_filters( 'woo_ctr_settings_args', wp_parse_args( $woo_ctr_settings, $args ) );
	}
